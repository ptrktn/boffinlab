...
# Deploy boffinlab Makefile
mkdir -p ${MY_TARGET}/usr/local/share/boffinlab
cat <<EOF | base64 -d \
	| gzip -dc > ${MY_TARGET}/usr/local/share/boffinlab/Makefile || true
H4sIAKG8LWAAA1NW8CvNyeECAD6w57cHAAAA
EOF

log_command_in_target sha1sum /usr/local/share/boffinlab/Makefile
log_command_in_target make BOFFIN=boffin -C /usr/local/share/boffinlab deps
log_command_in_target su - boffin -c "make -C /usr/local/share/boffinlab"
